...
function formular($titel='',$vorname='',$nachname='',$email='',$gebdatum='',$strasse='',$snr='',$plz='',$ort='',$nat='',$telefon='',$fax='',$newsletter=0,$jupi=0,$arm=0,$liquid=0,$comment='',$umfragen=0,$aktion=0)
{
?>
<FORM METHOD="POST">
<TABLE>
...
<TR> <TD>Möchtest Du allgemeine Mitgliederinformationen über die Piraten haben?</TD><TD><input type="checkbox" name="newsletter" value="1" <?if ($newsletter==1) echo "checked";?>/></TD></TR>
...
<TR> <TD>Möchtest Du an Umfragen teilnehmen?</TD><TD><input type="checkbox" name="umfragen" value="1"/ <?if ($umfragen==1) echo "checked";?>></TD></TR>
...
function leer($feld,$text,$titel='',$vorname='',$nachname='',$email='',$gebdatum='',$strasse='',$snr='',$plz='',$ort='',$nat='',$tel,$fax='',$newsletter=0,$jupi=0,$arm=0,$liquid=0,$comment='',$umfragen=0,$aktion=0)
{
   if($feld=="")
   {
	echo "<p class='statusmessage' style='color: red;' > $text darf nicht leer sein </p>";
	formular($titel,$vorname,$nachname,$email,$_POST["gebdatum"],$strasse,$snr,$plz,$ort,$nat,$tel,$fax,$news,$jupi,$arm,$liquid,$comment,$umfragen,$aktion);
	exit;
   }
}
...
else
{
   if($_POST["send"]==1)
   {
     $link=mysql_connect('localhost',$config['username'],$config['password']);
      ...
      if ($_POST["aktion"]=="1") $aktion=1; else $aktion=0;
      $gebdatum=date("Y-m-d",strtotime(secure($_POST["gebdatum"])));
        
        leer($vorname,"Vorname",$titel,$vorname,$nachname,$email,$_POST["gebdatum"],$strasse,$snr,$plz,$ort,$nat,$tel,$fax,$news,$jupi,$arm,$liquid,$comment,$umfragen,$aktion);
	leer($nachname,"Nachname",$titel,$vorname,$nachname,$email,$_POST["gebdatum"],$strasse,$snr,$plz,$ort,$nat,$tel,$fax,$news,$jupi,$arm,$liquid,$comment,$umfragen,$aktion);
	leer($strasse,"Strasse",$titel,$vorname,$nachname,$email,$_POST["gebdatum"],$strasse,$snr,$plz,$ort,$nat,$tel,$fax,$news,$jupi,$arm,$liquid,$comment,$umfragen,$aktion);
	leer($plz,"Postleitzahl",$titel,$vorname,$nachname,$email,$_POST["gebdatum"],$strasse,$snr,$plz,$ort,$nat,$tel,$fax,$news,$jupi,$arm,$liquid,$comment,$umfragen,$aktion);
	leer($ort,"Ort",$titel,$vorname,$nachname,$email,$_POST["gebdatum"],$strasse,$snr,$plz,$ort,$nat,$tel,$fax,$news,$jupi,$arm,$liquid,$comment,$umfragen,$aktion);
